...
class IPAdapterXLInpainting(IPAdapter):
    ...
    def load_ip_adapter(self):
        ...
        if os.path.splitext(self.ip_ckpt)[-1] == ".safetensors":
            ...
            # pdb.set_trace()
            self.pipe.unet.load_state_dict(state_dict["unet"], strict=True)
            print(f"loaded unet checkpoint.")
        # elif os.path.splitext(self.ip_ckpt)[-1] == ".pth":
            
        #     state_dict = {"image_proj": {}, "ip_adapter": {}}
        #     checkpoint = torch.load(self.ip_ckpt)
        #     pdb.set_trace()
        #     for key in checkpoint.keys():
        #         if key.startswith("image_proj_model."):
        #             state_dict["image_proj"][key.replace("image_proj_model.", "")] = checkpoint[key]
        #         elif key.startswith("unet."):
        #             state_dict["ip_adapter"][key.replace("unet.", "")] = checkpoint[key]
        # if os.path.splitext(self.ip_ckpt)[-1] == ".safetensors":
    ...
    def generate(
        ...
        **kwargs,
        ...
        with torch.inference_mode():
            ...
            print(f"negative_prompt_embeds: {negative_prompt_embeds}")
            print(f"pooled_prompt_embeds: {pooled_prompt_embeds.shape}")
            print(f"negative_pooled_prompt_embeds: {negative_pooled_prompt_embeds.shape}")
