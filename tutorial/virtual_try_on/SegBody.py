...
app = FaceAnalysis(root="weights/insightface", providers=['CUDAExecutionProvider', 'CPUExecutionProvider'])
app.prepare(ctx_id=0, det_size=(640, 640))
print(f"loaded insightface.")
# Initialize segmentation pipeline
# from pathlib import Path
# 指定本地模型路径
model_path = "tutorial/virtual_try_on/mattmdjaga/segformer_b2_clothes"
feature_extractor = SegformerFeatureExtractor.from_pretrained(model_path)
model = SegformerForSemanticSegmentation.from_pretrained(model_path)

segmenter = pipeline("image-segmentation", model=model, feature_extractor=feature_extractor)
print(f"loaded body segmenter.")
